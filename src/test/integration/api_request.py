...
def query_request(url, access_token, verb, query_params):
    ...
    request = requests.request(verb, url, params=query_params, headers=headers, verify='/tmp/api.pem')
    return request

def unauth_request(url):
    request = requests.get(url, verify='/tmp/api.pem')
    return request.status_code

def response_time(url, access_token):
    query_params = {'q': 'Oxford'}
    headers = {'Authorization': access_token}
    request = requests.get(url, params=query_params, headers=headers, verify='/tmp/api.pem')
